...
import { addFigure, listFigures, rememberFigure, rememberedFigure, removeFigure } from '../figures.js';
...
async function paint(entry) {
  ...
  const gltf = await parseFigure(await entry.blob.arrayBuffer());
  holder.clear();
...
function card(entry, active) {
  ...
  body.append(el('p', 'muted small', mapped + ' of 55 humanoid bones mapped'));
